...
| `gemini_client.py` | Wraps `google-genai` SDK. Four operations: `render()` (SketchUp → photorealistic, optional reference image), `enhance()` (quality upscale), `inpaint()` (local edit — mask OR text-only), `analyze_mood()` (text-only JSON response — no image modality). `_build_config()` constructs `GenerateContentConfig` defensively (tries full → partial → empty to survive SDK changes). **`render()` with a reference does TEXT-based style sync**: `describe_style()` (text-only) extracts the reference's materials/lighting as text, injected into the prompt via `REFERENCE_STYLE_SYNC`; the reference image itself is NOT sent, so geometry stays 100% from the SketchUp input (see Reference-image style sync). `describe_style()` is a 5th text-only operation. |
...
Bug class: when a polished render is passed as `reference_image`, Gemini gravitates to the **more photorealistic** image and **copies its composition** — so rendering camera-2's SketchUp with camera-1's render as reference returns a copy of camera-1 (wrong angle). Image-based mitigations (labelling `IMAGE 1`/`IMAGE 2`, then `_downscale_reference` to 512px) reduced but **never eliminated** dominance for detailed scenes.

**Final fix — JSON-structured text style sync (no reference image sent):**
- `gemini_client.describe_style()` runs a Gemini call with `STYLE_DESCRIPTION_PROMPT` that asks for a **strict JSON object** with 6 atomic material keys (`floor`, `walls`, `ceiling`, `accents`, `palette`, `lighting`) — forced via `response_mime_type="application/json"` (same proven mechanism as `analyze_mood`).
- **Critical:** the model's free-form text is NEVER injected. `_build_palette_text()` parses the JSON and **reconstructs** the palette string in Python from ONLY those 6 keys, discarding any other text. This is the fix for the recurring camera-drift bug: earlier versions sent the model's free-form caption, which embedded scene/spatial info ("walnut panelling **behind a long table**, **central** screen") that out-competed the weak clay geometry and flipped camera-2 back to camera-1's frontal view. Atomic material tokens (`FLOOR: dark walnut, warm brown`) carry zero composition, so the clay input fully determines the camera.
- If JSON parsing fails (model returned a caption instead), `_build_palette_text()` returns `""` → `render()` skips style sync entirely. **Intentional safe default: a correct angle with unmatched colours beats a wrong angle.**
- `render()` injects the reconstructed palette via `REFERENCE_STYLE_SYNC` and sends **only the SketchUp input image** — no reference image at all. So geometry/camera come 100% from the input (correct angle guaranteed); style transfers via the atomic palette text.
- Used by both the manual "Làm Reference" flow and multi-angle (angles 2/3 sync to angle-1's look). When a reference is present, `main.py` rebuilds the prompt with `use_reference_style=True` so cam-2's OWN style preset is dropped (it would otherwise conflict with cam-1's palette and cause partial drift). `_downscale_reference` and `REFERENCE_INSTRUCTION` remain defined but are **no longer used by `render()`**.

**Lighting-fixture clamp (chống AI tự thêm đèn thả):** interior style presets name specific fixtures (e.g. "Hiện đại" preset text includes *"oversized globe pendants in brushed brass"*). `_INTERIOR_STYLE_CLAMP` alone loses to such a concrete named description, so Gemini adds pendants the clay model never had. `_LIGHTING_FIXTURE_CLAMP` is appended **last** in `build_interior_prompt()` (strongest recency position) and explicitly overrides any pendant/chandelier wording: render only fixtures physically in the model; if the ceiling is bare, allow only recessed downlights + one cove LED, never a pendant/row of pendants. The pendant bug is NOT in `GEOMETRY_LOCK` — don't look there; it originates in the style preset text.
