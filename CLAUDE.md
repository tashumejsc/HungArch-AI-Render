...
**Final fix — text-based style sync (no reference image sent):**
- `gemini_client.describe_style()` runs a cheap **text-only** Gemini call (`STYLE_DESCRIPTION_PROMPT`) that extracts the reference render's **materials / colours / lighting** as a short text description (NOT its layout/camera).
- `render()` injects that text into the prompt via `REFERENCE_STYLE_SYNC` and sends **only the SketchUp input image** — no reference image at all. So geometry/camera come 100% from the input (correct angle guaranteed); style transfers via text. Costs one extra cheap text-only call per reference render; fails gracefully (skips style sync) if the description call errors.
- Used by both the manual "Làm Reference" flow and multi-angle (angles 2/3 sync to angle-1's look). `_downscale_reference` and `REFERENCE_INSTRUCTION` remain defined but are **no longer used by `render()`**.
