...
3. **`image_utils.overlay_linework()`** in `main.py` — **MULTIPLY blend**, the definitive anti-doubling design: `result = original_drawing × (flat_wash / 255)`. Structure comes 100% from the original (black line × anything = black; white room × colour = tinted room), so Gemini's drifted geometry **cannot appear** — doubling is impossible by construction, not merely suppressed. The wash is flattened first: HSV brightness-floor (`value_floor`, drops Gemini's dark walls/shadows) then a heavy Gaussian blur (dissolves every Gemini line into a smooth colour field). Earlier stamp-/erase-based versions only made the ghost fainter; multiply removes it entirely. Applied in both `/api/render` (drawing 2D) and `/api/render-pdf-page`.
